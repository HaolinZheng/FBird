...
public class GameScreen implements Screen {
    ...
    Array<Seed> seeds;
    long lastObstacleTime, lastFoodTime;
    float score;
    boolean augmentar = false;
    ;
    public GameScreen(final Bird gam) {
        ...
        spawnObstacle();
        lastFoodTime = TimeUtils.nanoTime() - 750000000;
        score = 0;
    }
    @Override
    public void render(float delta) {
        ...
        if (TimeUtils.nanoTime() - lastObstacleTime > 1500000000)
            spawnObstacle();
        if (TimeUtils.nanoTime() - lastFoodTime > 3000000000L)
            spawnFood();
        ...
        game.batch.begin();
        game.smallFont.draw(game.batch, "Fuerza: " + (int)player.fuerza, 10,
                400);
        game.batch.end();

        //La puntuació augmenta amb el temps de joc
        score += Gdx.graphics.getDeltaTime();
        player.restarImpulso(score);
        ...
        Iterator<Seed> iter2 = seeds.iterator();
        while (iter2.hasNext()) {
            Seed seed = iter2.next();
            if (seed.getBounds().overlaps(player.getBounds())) {
                seed.remove();
                seeds.removeValue(seed, true);
                if (augmentar && player.fuerza < 450){
                    player.fuerza += 10;
                    augmentar = false;
                }
            }
    ...
    private void spawnFood() {
        Seed seed = new Seed();
        float pos = MathUtils.random(50, 230);
        seed.setX(800);
        seed.setY(pos);
        seed.setManager(game.manager);
        seeds.add(seed);
        stage.addActor(seed);
        augmentar = true;
